import { Account, AccountManager, ContractDeployer, assertRowsEqualStrict, assertRowsEqual } from 'lamington';
...
describe('scope', function() {
	...
	it('should have correct defaults', async function() {
		await assertRowsEqualStrict(contract.players(), []);
		await assertRowsEqualStrict(contract.items(), []);
	});

	it('should allow a player to be added', async function() {
		await contract.signup(account1.name, { from: account1 });
		await assertRowsEqualStrict(contract.players(), [{ account: account1.name }]);
	});
	...
	it('should allow an item to be added', async function() {
		...
		await assertRowsEqualStrict(contract.items({scope:account1.name}), [{ id: 0, name: 'This is an item' }]);
		...
		await assertRowsEqual(contract.items({scope:account2.name}), [
